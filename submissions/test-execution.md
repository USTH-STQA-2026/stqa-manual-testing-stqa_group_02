...
| TC-14 | Search & Filter | Category filter should be case-insensitive | The system displayed all books correctly when the category `Công nghệ` was entered. However, entering `công nghệ` in lowercase returned no results. The filter did not treat uppercase and lowercase inputs equally as required. | Fail |  | BUG-07 |

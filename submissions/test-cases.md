...
| TC-18 | Borrow book - Member borrows for themself | Logged in as dam.tran. Refresh. | 1. Log in as Member. 2. Find an available book. 3. Click "Borrow". | Member: MEM003 / Book: BOOK001 | Borrow successful. Borrow record belongs to MEM003, not another member. | REQ-04 | EP |
| TC-19 | BVA - Borrow book when borrowCount = 0 (min boundary) | Logged in as dam.tran. Refresh. | 1. Confirm dam.tran has borrowed no books (count = 0). 2. Borrow BOOK001. | Member: MEM003 (count = 0) / Book: BOOK001 | Borrow successful. borrowCount changes from 0 to 1. This is lower boundary test. | REQ-04 | BVA |
...
| TC-23 | Member has no permission to click "Check Overdue" | Logged in as ba.nguyen (Member). | 1. Observe the interface after login. 2. Look for the "Check Overdue" button. | - | "Check Overdue" button is NOT visible in the Member interface. Only Librarian can see this button. | REQ-06 | EP, Permission |

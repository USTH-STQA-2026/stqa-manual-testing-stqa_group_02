...
| TC-23 | BVA — dueDate = today must be flagged as Overdue| Logged in as Librarian. Create a new borrow record with dueDate = today.| 1. Borrow a book for a member with today's date. 2. Manually set dueDate = today (if possible). 3. Click "Check Overdue".| dueDate = today's date| Record MUST be flagged as "Overdue" (SRS rule: dueDate ≤ today → Overdue). If record is NOT flagged → BUG-07.| REQ-06 | BVA, Permission |
